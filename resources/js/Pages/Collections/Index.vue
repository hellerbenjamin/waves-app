...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <Message v-if="!collections.length" severity="info" :closable="false">
            No collections yet. Create one to gather tracks, photos, and videos from across your events into a single shareable set.
        </Message>
        ...
        <ul v-else class="collection-list">
            ...
            >
                <div class="row-icon"><i class="pi pi-folder" /></div>
                <div class="row-body">
                    <div class="row-top">
                ...
                <div class="row-counts">
                    <span
                        class="count"
                        :title="`${collection.tracks_count} ${collection.tracks_count === 1 ? 'track' : 'tracks'}`"
                        :aria-label="`${collection.tracks_count} ${collection.tracks_count === 1 ? 'track' : 'tracks'}`"
                    >
                        <i class="pi pi-volume-up" aria-hidden="true" /> {{ collection.tracks_count }}
                    </span>
                    <span
                        class="count"
                        :title="`${collection.media_count} photos & videos`"
        ...
        <Dialog v-model:visible="showCreate" modal header="New collection" :style="{ width: '32rem' }">
            <div class="form">
                ...
                <div class="field">
                    <label for="c-desc">Description <span class="optional">(optional)</span></label>
...
.row-icon { flex: 0 0 auto; width: 2.5rem; text-align: center; color: var(--p-primary-color); font-size: 1.4rem; }
